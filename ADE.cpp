...
//获得opcode0指向的指令长度
static DWORD ade32_disasm(IN LPVOID opcode0)
{
    PBYTE opcode = (PBYTE)opcode0;
    DWORD flag = 0;
    BYTE c;
    DWORD t, a, d, i;
    disasm_struct diza;
...
DWORD ade_getlength(LPVOID Start, DWORD WantLength)
{
	DWORD CurLength;
	DWORD TotalLength = 0;
	...
	while ((CurLength = ade32_disasm(Start)) != 0)
	{
		if (TotalLength >= WantLength)
		{
			return TotalLength;
		}

		Start = (PBYTE)Start + CurLength;
