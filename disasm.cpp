...
static void Memadr(int defseg, const char* descr, long offset, int dsize)
{
	...
	if (showmemsize != 0 || (da->cmdtype & C_TYPEMASK) == C_MMX || (da->cmdtype & C_TYPEMASK) == C_NOW || (da->cmdtype & C_EXPL) != 0)
	{
...
static void DecodeMR(int type)
{
	...
	if (mode >= DISASM_DATA)
	{
		...
		case MR1:
			...
			dsize = regsize = 1;
			if (inmemory)
				da->memtype = DEC_BYTE; break;
		case MR2:
			// 2-byte memory/register in ModRM byte
			dsize = regsize = 2;
			if (inmemory)
				da->memtype = DEC_WORD; break;
		case MR4:
		...
		case MDA:
			...
			dsize = 10; memonly = 1;
			if (inmemory)
				da->memtype = DEC_TBYTE; break;
		case MF4:
			// Memory in ModRM byte (32-bit float)
			dsize = 4; memonly = 1;
			if (inmemory)
				da->memtype = DEC_FLOAT4; break;
		case MF8:
			// Memory in ModRM byte (64-bit float)
			dsize = 8; memonly = 1;
			if (inmemory)
				da->memtype = DEC_FLOAT8; break;
		case MFA:
			// Memory in ModRM byte (80-bit float)
			dsize = 10; memonly = 1;
			if (inmemory)
				da->memtype = DEC_FLOAT10; break;
		case MFE:
...
ulong Disasm(char* src, ulong srcsize, ulong srcip, t_disasm* disasm, int disasmmode)
{
	...
	if (repeated)
	{
		if (mode >= DISASM_FILE)
		{
			...
			{
			case 0x26:
				pname = (char*)(segname[SEG_ES]); break;
			case 0x2E:
				pname = (char*)(segname[SEG_CS]); break;
			case 0x36:
				pname = (char*)(segname[SEG_SS]); break;
			case 0x3E:
				pname = (char*)(segname[SEG_DS]); break;
			case 0x64:
				pname = (char*)(segname[SEG_FS]); break;
			case 0x65:
				pname = (char*)(segname[SEG_GS]); break;
			case 0x66:
				pname = "DATASIZE"; break;
			case 0x67:
				pname = "ADDRSIZE"; break;
			case 0xF0:
				pname = "LOCK"; break;
			case 0xF2:
				pname = "REPNE"; break;
	...
	else
	{
		...
		// 83,EC,imm8).
		if (cmd[0] == 0x44 || cmd[0] == 0x4C || (size >= 3 && (cmd[0] == 0x81 || cmd[0] == 0x83) && (cmd[1] == 0xC4 || cmd[1] == 0xEC) && (cmd[2] & 0x03) != 0))
		{
		...
		if ((pd->bits & WW) != 0 && ((BYTE)*cmd & WW) == 0)
			datasize = 1;   				   // Bit W in command set to 0
		else if ((pd->bits & W3) != 0 && ((BYTE)*cmd & W3) == 0)
		...
		for (operand = 0; operand < 3; operand++)
		{
			...
			case IMU:
				...
				if ((pd->bits & SS) != 0 && ((BYTE)*cmd & 0x02) != 0)
